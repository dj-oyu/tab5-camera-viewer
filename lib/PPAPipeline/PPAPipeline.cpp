...
void PPAPipeline::prepareConfig(ppa_srm_oper_config_t &config,
                                ...
                                float scale_x, float scale_y)
{
  ...
  uint32_t scaled_w = static_cast<uint32_t>(src_w * scale_x);
  uint32_t scaled_h = static_cast<uint32_t>(src_h * scale_y);
